...
const UsersPage = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await fetch('https://ycspout.umwalimu.com/api/users.php', {
        method: 'GET',
        headers: { 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = editingUser 
        ? `https://ycspout.umwalimu.com/api/users.php?id=${editingUser.id}`
        : 'https://ycspout.umwalimu.com/api/users.php';
  ...
  const handleDelete = async (userId) => {
    ...
    if (window.confirm('Are you sure you want to delete this user? This action cannot be undone.')) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://ycspout.umwalimu.com/api/users.php?id=${userId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
6. Access via https://ycspout.umwalimu.com/api/users.php`}
              </div>
