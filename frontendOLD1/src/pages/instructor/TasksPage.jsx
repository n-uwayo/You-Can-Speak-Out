...
const TasksPage = () => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          console.warn('Tasks API endpoint not found. Please implement /api/tasks endpoint in your backend.');
          setApiMissing(true);
          setTasks([]);
          return;
      ...
      setTasks(data?.data?.tasks || data?.tasks || data || []);
      setApiMissing(false); // API is working
    } catch (error) {
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setCourses(data?.data?.courses || data?.courses || data || []);
      } else {
  ...
  const fetchModules = async (courseId) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setModules(data?.data?.modules || data?.modules || data || []);
      } else {
  ...
  const fetchVideos = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setVideos(data?.data?.videos || data?.videos || data || []);
      } else {
  ...
  const handleEdit = (task) => {
    ...
    setFormData({
      ...
      moduleId: task.moduleId || moduleInfo.id || '',
          ...
          <div className="flex items-start">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-yellow-700 mb-4">
                Your backend doesn't have the tasks endpoints yet. You need to implement the following API routes:
              </p>
              
              <div className="bg-yellow-100 rounded-md p-3 mb-4">
                <div className="text-xs font-mono text-yellow-800 space-y-1">
                  <div><strong>GET</strong> /api/tasks - Fetch all tasks</div>
                  <div><strong>POST</strong> /api/tasks - Create new task</div>
                  <div><strong>PUT</strong> /api/tasks/:id - Update task</div>
                  <div><strong>DELETE</strong> /api/tasks/:id - Delete task</div>
                </div>
              </div>
    ...
    "tasks": [
      {
        ...
        "module": {
          "id": "module_id",
          "title": "Module Name",
          "course": { "id": "course_id", "title": "Course Name" }
        },
        "submissions": [...]
      }
    ]
              ...
              <div className="flex items-center space-x-3 text-sm">
                <span className="text-yellow-700">Based on your Prisma schema Task model</span>
                <button
