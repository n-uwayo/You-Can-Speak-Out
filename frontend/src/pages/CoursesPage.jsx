...
const CoursesPage = () => {
  ...
  // Helper function to extract YouTube ID (in case the youtube_url contains full URL)
      ...
      <div className="relative">
        <div className="w-full h-48 bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
          {playingVideo === course.id ? (
            // Embedded YouTube player when playing
            <iframe
              width="100%"
              height="100%"
              src={`https://www.youtube.com/embed/${getYouTubeId(course.youtube_url)}?autoplay=1`}
              title={course.title}
              frameBorder="0"
            ...
            <img
              src={`https://img.youtube.com/vi/${getYouTubeId(course.youtube_url)}/hqdefault.jpg`}
              alt={course.title}
              className="w-full h-full object-cover"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = `https://img.youtube.com/vi/${getYouTubeId(course.youtube_url)}/mqdefault.jpg`;
              }}
            />
          ) : course.thumbnail ? (
            <img
        ...
        {/* Play button overlay - now clickable */}
        {course.youtube_url && playingVideo !== course.id && (
          <button
            onClick={(e) => {
              e.stopPropagation();
          ...
          >
            <div className="bg-black bg-opacity-60 rounded-full p-3 transform transition-transform hover:scale-110">
        ...
        {playingVideo === course.id && (
          <button
            ...
            className="absolute top-2 right-2 bg-black bg-opacity-75 text-white p-1 rounded-full hover:bg-opacity-90 z-10"
          >
            <X className="w-4 h-4" />
          </button>
        )}
